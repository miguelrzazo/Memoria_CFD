...
\begin{itemize}
    ...
    \item Viscosidad cinemática: $\nu = \dfrac{U_\infty \cdot D}{Re} = \dfrac{1.0 \times 1.0}{550} = 1.818 \times 10^{-3}$ m$^2$/s
\end{itemize}

\textbf{Nota sobre configuración del caso:} El caso base proporcionado empleaba $\nu = 1.5 \times 10^{-3}$ m$^2$/s (correspondiente a $Re \approx 667$). Para cumplir con el requisito del enunciado de $Re = 550$ (calculado como $Re = 200 + l_D \times 50$ con $l_D = 7$), se ajustó la viscosidad a $\nu = 1.818 \times 10^{-3}$ m$^2$/s en el archivo \texttt{constant/physicalProperties}.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{Ejercicio7/Cd_vs_tiempo_con_zoom.png}
    \caption{Evolución temporal del coeficiente de arrastre $C_D$ con detalle de los últimos 5 segundos. Se indican el valor medio simulado ($\bar{C}_d = 0.898$) y el valor experimental de referencia según Roshko~\cite{roshko1954} ($C_d = 1.10$).}
...
El coeficiente de arrastre presenta un transitorio inicial (aproximadamente $t < 20$ s) durante el cual se establece el patrón de desprendimiento de vórtices. Una vez alcanzado el régimen cuasi-estacionario ($t > 20$ s), $C_D$ oscila alrededor de un valor medio:
\begin{equation}
...
El valor obtenido en la simulación ($\bar{C}_D = 1.30$) presenta un exceso del 18\% respecto al valor experimental medio de referencia ($C_D^{\text{exp}} = 1.10$). Esta discrepancia puede atribuirse a:
\begin{itemize}
    \item \textbf{Modelo URANS vs. flujo real:} La simulación emplea el modelo $k$-$\omega$ SST URANS, que promedia temporalmente las fluctuaciones turbulentas. Este enfoque puede sobrestimar el arrastre respecto a simulaciones LES o DNS que resuelven explícitamente las escalas turbulentas.
    \item \textbf{Bidimensionalidad:} La simulación 2D no captura efectos tridimensionales como el estiramiento vortical en dirección axial, que en flujos reales reduce la coherencia de los vórtices y modifica el arrastre.
    \item \textbf{Efectos finales:} Los experimentos tienen relación de aspecto finita (efectos de pared lateral), mientras que la simulación 2D asume envergadura infinita.
    \item \textbf{Incertidumbre experimental:} Los datos de Roshko~\cite{roshko1954} presentan dispersión en este rango de Reynolds debido a la sensibilidad del flujo a perturbaciones y condiciones de entrada.
\end{itemize}

No obstante, el orden de magnitud es correcto ($C_D \approx 1.3$ vs. $C_D^{\text{exp}} \approx 1.1$) y las oscilaciones temporales son coherentes con el fenómeno físico esperado.
...
La amplitud de las oscilaciones es aproximadamente $C_L \in [-0.12, +0.12]$, reflejando la alternancia del desprendimiento de vórtices entre los lados superior e inferior del cilindro. La Fig.~\ref{fig:Cd_Cl_detalle_ej7} muestra un detalle de las oscilaciones durante los últimos 10 segundos de simulación.
...
\begin{equation}
    f_{\text{dominante}} = 0.1667 \text{ Hz}
\end{equation}

Por lo tanto, el número de Strouhal simulado es:
\begin{equation}
    St = \frac{f \cdot D}{U_\infty} = \frac{0.1667 \times 1.0}{1.0} = 0.1667
\end{equation}
...
El valor obtenido en la simulación ($St = 0.167$) presenta una desviación del 20.6\% respecto al valor experimental ($St^{\text{exp}} = 0.21$). Esta diferencia puede explicarse por:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Coeficiente de arrastre:} El valor medio del coeficiente de arrastre obtenido es $\bar{C}_D = 1.30$, lo cual representa un exceso del 18\% respecto al valor experimental de Roshko~\cite{roshko1954} ($C_D^{\text{exp}} \approx 1.10$ para Re=550). Esta discrepancia es atribuible al modelo URANS $k$-$\omega$ SST, que puede sobrestimar el arrastre en comparación con simulaciones que resuelven explícitamente las escalas turbulentas (LES/DNS), y a efectos de bidimensionalidad.

    \item \textbf{Número de Strouhal:} El análisis espectral mediante FFT de la señal de $C_L(t)$ ha permitido determinar la frecuencia de desprendimiento $f = 0.1667$ Hz, resultando en un número de Strouhal $St = 0.167$. Este valor presenta una desviación del 20.6\% respecto al valor experimental~\cite{roshko1954,williamson1996vortex} ($St^{\text{exp}} \approx 0.21$), lo cual puede atribuirse a las limitaciones del modelo URANS para capturar con precisión la dinámica de estructuras coherentes como los vórtices de von Kármán.

    \item \textbf{Régimen de flujo:} La simulación URANS con el modelo $k$-$\omega$ SST captura correctamente el fenómeno de desprendimiento periódico de vórtices a Re=550, proporcionando resultados cualitativamente correctos. Sin embargo, para predicciones cuantitativas más precisas en este régimen de transición, se recomendaría emplear simulaciones 3D con modelos más sofisticados (LES) o resolución directa (DNS).
